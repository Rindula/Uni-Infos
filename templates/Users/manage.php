        ...
        <table>
            ...
            <?php foreach ($users as $user): ?>
                ...
                <?= $this->Html->tableCells([
                    ...
                    h($user->email),
                    h($user->enabled) ?? '---',
                    (($user->enabled) ? '' : $this->Html->link(__('Send activation email'), ['action' => 'userAction', $user->id, 'sendmail'])) . $this->Form->submit('Benutzer speichern') . $this->Html->link('Benutzer deaktivieren', ['action' => 'disable', $user->id])
                ]) ?>
                <?= $this->Form->end(); ?>
            <?php endforeach; ?>
            </tbody>
